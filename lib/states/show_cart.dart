...
class _ShowCartState extends State<ShowCart> {
  ...
  Future<Null> procressReadSQLite() async {
    if (sqliteModels.isNotEmpty) {
      sqliteModels.clear();
    }

    await SQLiteHelper().readSQLite().then((value) {
      // print('### value on procressReadSQLite ==>> $value');
      setState(() {
        load = false;
        sqliteModels = value;
        findDetailSeller();
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: ListTile(
          ...
          title: ShowTitle(
              title: 'ເຈົ້າຕ້ອງການຈະ Delete ?',
              textStyle: MyConstant().h2BlueStyle()),
          subtitle: ShowTitle(
              title: 'Product ທັງໝົດ ໃນກະຕ່າ ແມ່ນບໍ ?',
